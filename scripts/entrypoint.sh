...
# ---------- 校验必要环境变量 ----------
REQUIRED_VARS=("FEISHU_WEBHOOK_TOKEN" "AWS_ACCESS_KEY_ID" "AWS_SECRET_ACCESS_KEY" "AWS_DEFAULT_REGION" "ZONE_ID" "RECORD_NAMES")
...
echo "[INFO] IP Cache File     : ${IP_CACHE_FILE:-/host-tmp/new_public_ip.txt}"
...
export IP_CACHE_FILE="${IP_CACHE_FILE:-/host-tmp/new_public_ip.txt}"
export TZ="${TZ:-Asia/Shanghai}"
export HOME="/root"
export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
EOF
chmod 600 "$ENV_FILE"
echo "[INFO] Environment exported to ${ENV_FILE}"

# ---------- 初始化日志 ----------
mkdir -p /app/logs
touch /app/logs/route53.log
...
cat > "$CRON_FILE" <<EOF
# AWS Route53 Auto-Update Daemon
# Schedule: ${CRON_SCHEDULE} (Asia/Shanghai)
